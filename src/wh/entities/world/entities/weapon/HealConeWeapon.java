...
public class HealConeWeapon extends Weapon{
    ...
    public void addStats(UnitType u, Table t){
        super.addStats(u, t);
        t.row();
        if(bullet instanceof HealCone b && b.healAmount > 0){
            t.add("[lightgray]" + Core.bundle.format("wh-bullet.secondHealAmount", Strings.autoFixed(b.healAmount, 2)));
            t.row();
        }
    ...
    protected boolean checkTarget(Unit unit, Teamc target, float x, float y, float range){
        return
        !(target.within(unit, range + unit.hitSize / 2) && target.team() == unit.team &&
        (target instanceof Healthc && ((Healthc)target).damaged() && ((Healthc)target).isValid()));
    }
