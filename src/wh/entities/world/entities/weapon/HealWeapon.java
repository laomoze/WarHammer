...
public class HealWeapon extends Weapon{
    ...
    {
        mountType = HealWeaponMount::new;
        targetInterval = 60f;
        targetSwitchInterval = 70f;
    }

    @Override
    protected Teamc findTarget(Unit unit, float x, float y, float range, boolean air, boolean ground){
        Building damagedBuilding = Units.findAllyTile(unit.team, x, y, range, Building::damaged);
        if(damagedBuilding != null) return damagedBuilding;
        else return super.findTarget(unit, x, y, range, air, ground);
    }

    @Override
    protected boolean checkTarget(Unit unit, Teamc target, float x, float y, float range){
        boolean damaged = !(target.within(unit, range) && target.team() == unit.team
        && target instanceof Building u && u.damaged() && u.isValid());
        if(target instanceof Building b && b.team == unit.team && b.damaged() && b.isValid()){
            return damaged;
        }else{
    ...
    public void update(Unit unit, WeaponMount m){
        if(!(m instanceof HealWeaponMount mount)) return;
        ...
        //find a new target 111
        if(!(unit.controller() instanceof Player) || (!controllable && autoTarget)){
    ...
    public static class HealWeaponMount extends WeaponMount{
        public @Nullable Building damagedBuilding;

        public HealWeaponMount(Weapon weapon){
            super(weapon);
        }
    }
}
