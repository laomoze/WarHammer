...
import wh.entities.bullet.laser.LightingPointLaserBulletType.PLBullet;

import static mindustry.Vars.headless;

public class PointLaserWeapon extends Weapon{
    public float chargeTime = 20f;

    public PointLaserWeapon(String name){
        super(name);
    }
    ...
    public void update(Unit unit, WeaponMount mount){
        ...
        if(continuous && mount.bullet != null){
            if(!mount.bullet.isAdded() || mount.bullet.time >= mount.bullet.lifetime || mount.bullet.type != bullet){
                ...
                if(mount.bullet instanceof PLBullet p){
                    p.chargeTimer = Mathf.approachDelta(p.chargeTimer, 1, unit.reloadMultiplier / chargeTime);
                }
                ...
                Tmp.v1.trns(shootAngle, shootLength).add(bulletX, bulletY);

                mount.bullet.aimX = Tmp.v1.x;
                mount.bullet.aimY = Tmp.v1.y;

                if(alwaysContinuous && mount.shoot && Angles.within(rotate ? mount.rotation : unit.rotation + baseRotation, mount.targetRotation, shootCone)){
