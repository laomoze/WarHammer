...
public class DrawParticleFlow extends DrawBlock {
    ...
    public Floatf<Float> curve = b -> Mathf.curve(b, 0f, 0.3f);
    ...
    public void draw(Building build) {
        ...
        if (build.warmup() > 0f && color.a > 0.001f) {
            ...
            for (int i = 0; i < particles; i++) {
                ...
                Tmp.v1.set((endX - startX) * fin * rand.random(0.5f, 1), (endY - startY) * rand.random(0.5f, 1) * fin).add(startX, startY).rotate(getRot(build));
                Tmp.v2.trns(shiftAng, shift).add(Tmp.v1).add(build);
                Draw.color(color);
                Draw.alpha(particleAlpha * build.warmup());
                Lines.lineAngle(Tmp.v2.x, Tmp.v2.y, realAng, length * curve.get(fout));
            }
            Draw.reset();
        }
    }
