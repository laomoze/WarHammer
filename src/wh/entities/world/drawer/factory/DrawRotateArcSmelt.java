...
public class DrawRotateArcSmelt extends DrawArcSmelt {
    public float flameX = 0, flameY = 0;
    public float rotateAngle = 20f;
    public Interp rotateInterp = Interp.linear;
    ...
    public void draw(Building build) {
        if (build.warmup() > 0f && flameColor.a > 0.001f) {
            ...
            for (int i = 0; i < particles; i++) {
                ...
                float angle = rand.random(360f) + rand.random(-rotateAngle, rotateAngle) * rotateInterp.apply(fin);
                float len = particleRad * Interp.pow2Out.apply(fin);
                Lines.stroke(particleStroke * build.warmup() * fout * Mathf.curve(fout, 0, 0.15f));
