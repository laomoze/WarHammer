...
void main() {
    ...
    for (int i = 0; i < MAX_SHOCKWAVES; i++) {
        ...
        if (abs(dst - radius) <= WAVE_RADIUS) {
            ...
            // Add a subtle convex lens effect: sample slightly closer to the wave center,
            // which creates local magnification inside the shock ring.
            float lensFactor = LENS_STRENGTH * pdiff * strength;
            displacement -= (worldCoords - wave.xy) * lensFactor / (radius * u_resolution);
        }
